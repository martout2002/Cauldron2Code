...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let repository;
    try {
      // Generate description based on project structure
      const projectType = scaffoldConfig.projectStructure === 'fullstack-monorepo' 
        ? 'Full-stack monorepo'
        : scaffoldConfig.projectStructure === 'nextjs-only'
        ? 'Next.js'
        : scaffoldConfig.projectStructure === 'react-spa'
        ? `${scaffoldConfig.frontendFramework} SPA`
        : 'Express API';
      
      repository = await repoService.createRepository({
        name,
        description: description || `Generated by StackForge - ${projectType} project`,
    ...
    const configWithFramework = addFrameworkProperty(scaffoldConfig);
